...
import {AUTH_RESETPASS_SAGAS, AUTH_VERIFY_EMAIL_SAGAS, AUTH_VERIFY_CODE_SAGAS} from 'store/actions';
...
function ResetPasswordContainer(props) {
   ...
   * @param {*} interval * int 
   * @param {*} setValue * setState // 변경되는 시간 상태값을 받을 state 
   *            
   */
  ...
  const handleSubmit = ({type, name, email, password, inputAuthCode, checkPassword}) => {
    if(type === 'sendEmail'){
      if(regEmail(email)){
        const verifyForm = {
          email: email
        ...
        AUTH_VERIFY_EMAIL_SAGAS(verifyForm);
        setValid(draft => {
  ...
  },[valid.check.password, valid.check.checkPassword,valid.check.attempt]);
  ...
  }, [resetSucc, resetEmailR, valid.check.attempt]);
