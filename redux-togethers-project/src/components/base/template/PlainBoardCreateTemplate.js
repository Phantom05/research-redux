...
const Styled ={
  PlainBoardCreateTemplate:styled.div`
    position: fixed;
    .notice__title{
      ${font};
      color:${color.white};
...
class PlainBoardCreateTemplate extends Component {
  render() {
    const {handleClickWriteBtn} = this.props;
    return (
      <Styled.PlainBoardCreateTemplate>
        <div>
          ...
          <span className={cx('write__box')} onClick={()=>handleClickWriteBtn('write')}>
            <Icon type="edit" />
          </span>
        </div>
...
export default PlainBoardCreateTemplate;